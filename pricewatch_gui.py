...
class PricewatchGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Pricewatch GUI")
    ...
    def create_widgets(self):
        ...
        self.theme_btn = ttk.Button(self.main_frame, text="Theme",
        ...
        ttk.Radiobutton(self.main_frame, text="Above", variable=self.mode_var,
                        value="above").grid(row=1, column=1, sticky=tk.W)
        ttk.Radiobutton(self.main_frame, text="Below", variable=self.mode_var,
                        value="below").grid(row=2, column=1, sticky=tk.W)
        ttk.Radiobutton(self.main_frame, text="Volatility",
                        variable=self.mode_var, value="vol").grid(
                            row=3, column=1, sticky=tk.W)

        ttk.Label(self.main_frame, text="Target (price or pct-mins):").grid(
            row=4, column=0, sticky=tk.W)
        self.target_var = tk.StringVar(value="100000")
        ttk.Entry(self.main_frame, textvariable=self.target_var,
                  width=30).grid(row=4, column=1, pady=5)
        ...
        ttk.Button(self.main_frame, text="Browse",
                   command=self.browse_wav).grid(row=5, column=2, padx=5)

        ttk.Button(self.main_frame, text="Start Monitoring",
                   command=self.start_monitoring).grid(
                       row=6, column=0, pady=10)
        ttk.Button(self.main_frame, text="Stop Monitoring",
    ...
    def apply_theme(self):
        ...
        style.configure("TRadiobutton", background=bg_color,
                        foreground=fg_color, indicatorcolor=entry_bg)
        ...
        self.console.configure(
            bg=console_bg, fg=console_fg, insertbackground=fg_color,
